...
public class DefaultConsulApiClient implements ConsulApiClient
{
  ...
  public void registerService(DiscoveryDruidNode node) throws Exception
  {
    ...
    if (config.getServiceTags() != null) {
      for (java.util.Map.Entry<String, String> e : config.getServiceTags().entrySet()) {
    ...
    long intervalSeconds = Math.max(1, config.getHealthCheckInterval().getStandardSeconds());
    long ttlSeconds = Math.max(30, intervalSeconds * 3);
  ...
  {
    consulClient.agentCheckPass("service:" + serviceId, note, config.getAclToken());
  }
